  ...
  const weeklyTrendsData = useMemo(() => {
    ...
    const completedByDay = days.map((day) => {
      ...
      return tasks.filter((task) => {
        return (
          task.completed && task.date && format(new Date(task.date), 'yyyy-MM-dd') === dayString
        );
      }).length;
    });
    ...
    const dueByDay = days.map((day) => {
      const dayString = format(day, 'yyyy-MM-dd');
      return tasks.filter((task) => {
        return task.dueDate && format(new Date(task.dueDate), 'yyyy-MM-dd') === dayString;
      }).length;
    });
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <Button
              ...
              aria-label={
                isMinimized ? 'Expand task statistics section' : 'Collapse task statistics section'
              }
            >
              {isMinimized ? (
                <>
            ...
            <TabsContent value="weekly" className="mt-6">
              <ChartContainer config={weeklyChartConfig} className="h-[300px]">
                <BarChart data={weeklyTrendsData}>
                  ...
                  <Bar
                    dataKey="completed"
                    fill={weeklyChartConfig.completed.color}
                    name="Completed"
                    radius={[4, 4, 0, 0]}
                  />
                  <Bar
                    dataKey="due"
                    fill={weeklyChartConfig.due.color}
                    name="Due"
                    radius={[4, 4, 0, 0]}
                  />
                </BarChart>
